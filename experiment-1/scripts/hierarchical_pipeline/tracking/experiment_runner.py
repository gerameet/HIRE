...
from ..visualization import plot_interactive_graph, overlay_masks, overlay_masks_with_ids
...
class ExperimentRunner:
    ...
    def run_full_pipeline(self) -> ExperimentRun:
        ...
        try:
            ...
            image_files = self._load_image_list(images_path, data_config.get("max_images"))
    ...
    def _load_image_list(
        ...
        if images_path.is_file():
            ...
            images = [
                img for img in sorted(images_path.iterdir())
    ...
    def _save_embeddings(self, parts: List, output_path: Path) -> None:
        ...
        embeddings = [p.embedding for p in parts if hasattr(p, "embedding") and p.embedding is not None]
        
        if embeddings:
            np.savez_compressed(
                output_path,
                embeddings=np.stack(embeddings),
                part_ids=[p.id for p in parts if hasattr(p, "embedding") and p.embedding is not None]
            )
    ...
    def _run_retrieval_eval(
        ...
        from ..evaluation.retrieval import PartRetrievalEngine, precision_at_k, recall_at_k
        ...
        metrics = {
            f"retrieval_precision@{top_k}": sum(precisions) / len(precisions) if precisions else 0.0,
            f"retrieval_recall@{top_k}": sum(recalls) / len(recalls) if recalls else 0.0,
