    ...
    for idx, metric in enumerate(metrics_to_plot):
        ...
        fig.add_trace(
            go.Bar(
                ...
                textposition="auto",
                showlegend=False,
                marker=dict(color=df[metric], colorscale="Viridis", showscale=False),
            ),
            row=row,
            col=col,
        )
...
def generate_comparison_report(
    experiment_ids: List[str], output_dir: Path, tracker=None
) -> None:
    ...
    html_report = _generate_html_report(experiments, metrics_df, config_diff, viz_dir)
...
def create_experiment_timeline(experiments: List[Dict[str, Any]], output_path: str):
...
def _generate_config_diff(experiments: List) -> str:
    ...
    for i, exp in enumerate(experiments[1:], 1):
        ...
        lines.append(
            f"## {base_exp.name if hasattr(base_exp, 'name') else 'Exp 1'} vs "
...
def _generate_html_report(
    experiments: List, metrics_df: pd.DataFrame, config_diff: str, viz_dir: Path
) -> str:
