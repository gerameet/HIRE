...
def plot_embedding_space(
    ...
    **kwargs,
    ...
    try:
        ...
        raise ImportError(
            "plotly required for embedding visualization. Install with: pip install plotly"
        )
    ...
    fig = px.scatter(
        ...
        color_continuous_scale=(
            "Viridis" if isinstance(color_data[0], (int, float)) else None
        ),
    )

    fig.update_traces(marker=dict(size=8, opacity=0.7))
    fig.update_layout(hovermode="closest", width=1000, height=800, font=dict(size=12))
...
def plot_embedding_clusters(
    ...
    **kwargs,
    ...
    try:
        ...
        raise ImportError(
            "scikit-learn and plotly required. Install with: pip install scikit-learn plotly"
        )
...
def _reduce_dimensions(
    embeddings: np.ndarray, method: str = "umap", n_components: int = 2, **kwargs
) -> np.ndarray:
    ...
    if method == "umap":
        ...
        reducer = PCA(
            n_components=n_components, random_state=kwargs.get("random_state", 42)
        )
        coords = reducer.fit_transform(embeddings)
        ...
        raise ValueError(
            f"Unknown reduction method: {method}. Use 'umap', 'tsne', or 'pca'."
        )
