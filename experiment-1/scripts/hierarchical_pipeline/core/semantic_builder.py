...
class SemanticHierarchyBuilder(BottomUpHierarchyBuilder):
    ...
    def __init__(self, config: Dict[str, Any], embedder: Optional[EmbeddingMethod] = None):
    ...
    def _initialize_semantic_tools(self):
        ...
        if self.aligner is None:
            if not self.embedder or not isinstance(self.embedder, CLIPEmbedding):
                 logger.warning("SemanticHierarchyBuilder requires CLIPEmbedding. Degrading to spatial only.")
                 return
    ...
    def build_hierarchy(self, parts: List[Part]) -> ParseGraph:
        ...
        for p in parts:
            node = Node(
                ...
                part=p, 
                level=0,
                concept=part_concepts.get(p.id)
            )
            graph.add_node(node)
        ...
        for j in range(n): # Child candidate
            ...
            for i in range(n): # Parent candidate
                ...
                # Semantic score
                semantic_score = 0.0
                if self.wordnet and p.id in part_concepts and parts[i].id in part_concepts:
                    ...
                    if self.wordnet.is_hypernym(parent_concept, child_concept):
                        ...
                        # Soft similarity?
                        semantic_score = self.wordnet.semantic_similarity(parent_concept, child_concept)
