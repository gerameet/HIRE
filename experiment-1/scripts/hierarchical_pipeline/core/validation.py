...
def validate_image(image: np.ndarray, name: str = "image") -> None:
    ...
    if image.dtype not in [np.uint8, np.float32, np.float64]:
        raise ValueError(
            f"{name} must be uint8 or float type, got {image.dtype}"
        )
...
def validate_embedding(
    ...
    if embedding.ndim != 1:
        raise ValueError(
            f"{name} must be 1D vector, got shape {embedding.shape}"
        )
    ...
    if embedding.dtype not in [np.float32, np.float64]:
        raise ValueError(
            f"{name} must be float type, got {embedding.dtype}"
        )
...
def validate_bbox(
    ...
    if not isinstance(bbox, (tuple, list)) or len(bbox) != 4:
        raise ValueError(
            f"{name} must be 4-tuple (x1, y1, x2, y2), got {bbox}"
        )
