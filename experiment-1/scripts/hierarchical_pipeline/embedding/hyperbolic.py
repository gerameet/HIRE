...
try:
    ...
    HAS_GEOOPT = False
    logger.warning("geoopt not available. Hyperbolic projections will use simple implementations.")
...
class HyperbolicDistance:
    ...
    def _poincare_distance(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if norm_x_sq >= 1.0 or norm_y_sq >= 1.0:
            logger.warning(f"Point outside Poincaré ball: ||x||²={norm_x_sq}, ||y||²={norm_y_sq}")
    ...
    if model == "poincare":
        ...
        for i, x in enumerate(embeddings):
            ...
            if abs(lorentz_norm + 1.0) > 1e-3:
                logger.error(f"Lorentz constraint violated at {i}: ⟨x,x⟩_L = {lorentz_norm}")
                return False
        return True
