...
class PartRetrievalEngine:
    ...
    def add_parts(self, parts: List[Any]) -> None:
        ...
        logger.info(f"Added {len(valid_parts)} parts to index (total: {len(self.parts)})")
    ...
    def query(self, query_part: Any, top_k: int = 5) -> List[Tuple[Any, float]]:
        ...
        if self.use_faiss:
            # FAISS search
            distances, indices = self.index.search(query_emb, min(top_k, len(self.parts)))
            results = [
    ...
    def query_batch(self, query_parts: List[Any], top_k: int = 5) -> List[List[Tuple[Any, float]]]:
...
def precision_at_k(results: List[Tuple[Any, float]], ground_truth: List[Any], k: int = 5) -> float:
    """Compute Precision@K.
...
def recall_at_k(results: List[Tuple[Any, float]], ground_truth: List[Any], k: int = 5) -> float:
    """Compute Recall@K.
...
def mean_average_precision(all_results: List[List[Tuple[Any, float]]], 
                           all_ground_truth: List[List[Any]]) -> float:
