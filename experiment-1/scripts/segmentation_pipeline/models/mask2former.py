...
class Mask2FormerSegmentationModel(SegmentationModel):
    ...
    def __init__(self, config: ModelConfig):
        ...
        if self.task not in ["instance", "semantic", "panoptic"]:
            raise ValueError(
                f"Invalid task '{self.task}'. Must be instance/semantic/panoptic"
            )
        self.processor = None
        self.model = None
    ...
    def initialize(self) -> None:
        ...
        checkpoint = (
            self.config.checkpoint or "facebook/mask2former-swin-base-coco-instance"
        )
    ...
    def _post_process_semantic(self, outputs, target_size) -> List[Segment]:
        ...
        for label_id in np.unique(segmap):
            ...
            segments.append(
                Segment(
                    mask=mask, bbox=bbox, score=1.0, label=label, label_id=int(label_id)
                )
            )
    ...
    def _post_process_panoptic(self, outputs, target_size) -> List[Segment]:
        ...
        for info in segments_info:
            ...
            label_id = info.get("label_id")
            label = (
                self._get_label_name(label_id) if label_id is not None else "unknown"
            )
            score = float(info.get("score", 1.0))
