...
class SAMSegmentationModel(PromptableSegmentationModel):
    ...
    def initialize(self) -> None:
        ...
        if self.use_automatic:
            ...
            self.mask_generator = SamAutomaticMaskGenerator(
                self.sam_model, **generator_params
            )
    ...
    def _segment_prompted(
        self, np_img: np.ndarray, prompts: Dict[str, Any]
    ) -> List[Segment]:
        ...
        boxes_tensor = None
        if boxes:
            boxes_tensor = self.torch.tensor(
                boxes, device=device, dtype=self.torch.float32
            )

        points_tensor = None
        if points:
            points_tensor = self.torch.tensor(
                points, device=device, dtype=self.torch.float32
            )

        labels_tensor = None
        if point_labels:
            labels_tensor = self.torch.tensor(
                point_labels, device=device, dtype=self.torch.int32
            )
        ...
        for i, mask in enumerate(masks):
            ...
            segments.append(
                Segment(mask=seg_mask, bbox=bbox, score=score, label="sam_prompt")
            )
    ...
    def segment_with_points(
        ...
        return self.segment(image, {"points": points, "point_labels": point_labels})
