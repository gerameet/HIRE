...
class SegmentationPipeline:
    ...
    def run_on_image_list(self, list_file: str) -> Dict[str, Any]:
        ...
        if self.model.uses_gpu() and self.config.num_workers > 1:
            print(f"⚠️  GPU model detected, reducing workers to 1 to avoid CUDA context issues")
    ...
    def run_on_single_image(self, image_path: str, output_dir: Optional[str] = None) -> SegmentationResult:
    ...
    def _print_summary(self) -> None:
        ...
        print(f"Avg segments/img:  {self.stats['total_segments']/max(1, self.stats['successful']):.1f}")
        print(f"Total time:        {self.stats['total_time']:.2f}s")
        print(f"Avg time/img:      {self.stats['total_time']/max(1, self.stats['successful']):.3f}s")
        print(f"{'='*70}")
