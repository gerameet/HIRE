...
class TestSemanticHierarchy(unittest.TestCase):
    ...
    def test_concept_aligner(self):
        ...
        def get_text_features(**kwargs):
            return torch.tensor(
                np.random.rand(len(kwargs["input_ids"]), 512).astype(np.float32)
            )

        import torch

        mock_model.get_text_features.side_effect = lambda **kwargs: torch.randn(
            2, 512
        )  # 2 concepts
        ...
        aligner._encode_text = MagicMock(
    ...
    def test_semantic_builder(self, MockAligner, MockWordNet):
        ...
        with patch(
            "hierarchical_pipeline.core.semantic_builder.CLIPEmbedding",
            DummyCLIPEmbedding,
        ):
            ...
            with patch(
                "hierarchical_pipeline.core.builder.pairwise_overlap_matrix"
            ) as mock_pom:
