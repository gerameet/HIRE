    ...
    with seg_model as model:
        ...
        if args.embedding_method:
            ...
            emb_config = {
                ...
                "cache_dir": (
                    args.embedding_cache if args.embedding_cache else "cache/embeddings"
                ),
                "use_cache": True,
            }
        ...
        for img_path in images:
            ...
            try:
                ...
                if embedding_method:
                    ...
                    print(
                        f"  Embeddings generated (dim={parts[0].embedding.shape[0] if parts else 'N/A'})"
                    )
...
def main():
    ...
    p_process.add_argument(
        "--embedding-method",
        help="Method for generating embeddings (e.g., clip, dinov2)",
    )
