...
from hierarchical_pipeline.adapters import SegmentationAdapter, SegmentationDiscoveryAdapter
...
def test_adapter_with_sam():
    ...
    sam_checkpoint = Path(__file__).parent / "hierarchical_pipeline" / "models" / "sam_vit_b_01ec64.pth"
    ...
    if len(parts) > 0:
        ...
        for part in parts:
            assert part.mask.shape == (image_np.shape[0], image_np.shape[1]), \
                f"Mask shape mismatch: {part.mask.shape} vs {image_np.shape[:2]}"
            assert part.mask.dtype in [np.bool_, np.uint8], \
...
def test_discovery_adapter():
    ...
    discovery = SegmentationDiscoveryAdapter(
        dummy_model,
        config={"min_confidence": 0.5, "min_area": 100}
    )
